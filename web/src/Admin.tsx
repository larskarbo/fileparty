...
function Admin({ user }) {
  ...
  useEffect(() => {
    ...
    //   setData(data)
    // })
    console.log('moment(new Date()).subtract(7, "days").milliseconds():', moment(new Date()).subtract(7, "days").valueOf())

    db.ref("/boards")
      .orderByChild("counter")
      .limitToLast(10)
      // .startAt(moment(new Date()).subtract(7, "days").valueOf())
      .once("value")
      .then((snapshot) => {
  ...
  return (
    <div
      className={
        "flex flex-col p-12 flex-grow rounded bg-white  border border-gray-300 shadow-lg "
      }
    >
        ...
        <div className="flex flex-col mr-8 flex-grow text-xs mb-4 rounded-xl shadow border border-blue-100 p-4">
          <div>
            ...
            <span className="text-blue-400 font-bold ml-2">
              {stats.dates[moment().format("YYYY-MM-DD")]?.boardsCreated || 0}
            </span>
          </div>
          <Chart
            width={"400px"}
        ...
        <div className="flex flex-col flex-grow text-xs mb-4 rounded-xl shadow border border-blue-100 p-4">
          <div>
            <span className="uppercase font-bold">
              Boards created yesterday:
            </span>
            <span className="text-blue-400 font-bold ml-2">
              {stats.dates[moment().subtract(1, "day").format("YYYY-MM-DD")]
                ?.boardsCreated || 0}
            </span>
          </div>
          <Chart
      ...
      <div className="text-xs mb-4 rounded-xl shadow border border-blue-100 p-4">
        <span className="uppercase font-bold">
          10 most popular boards (7 days):
        </span>

        <div className="mt-4">
          <ul>
            {Object.entries(featuredBoards).map(
                  ...
                  <li key={key} className="flex flex-row">
                    ...
                    <div className="w-32">
                      ...
                      <Popover className="relative">
                        <Popover.Button>
                          {Object.keys(value.items || []).length || "No files"}
                        </Popover.Button>

                        <Popover.Panel className="absolute z-10 w-96 bg-white p-10">
                          {Object.values(value.items || []).map((item: any) => (
                    ...
                    <div className="w-32">
                      Date: {moment(value.created).format("MMM Do")}
                    </div>
                  </li>
                );
              }
